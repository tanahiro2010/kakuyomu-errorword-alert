...
const episodeButtons: HTMLParagraphElement | null = document.querySelector('#contentMainHeader-right');
...
interface ErrorReport {
    ...
    work_id: string;
    episode_id: string;
    error: string; // JSON文字列 (ChangedParagraph[])
    comment: string | null;
    edited: number | null;
}
...
async function handleEpisodeButtonClick() {
    // モーダルダイアログを作成
    await createEpisodeErrorReportModal();
}

async function handleEpisodeResolveError(workId: string, episodeId: string, errorId: string) {

    try {
        const response = await fetch(`${EpisodeApiBaseUrl}/works/${workId}/episodes/${episodeId}/errors/${errorId}/edit`, {
            method: 'PATCH'
        });
...
async function createEpisodeErrorReportModal() {
    // 既存のモーダルがあれば削除
    ...
    // 閉じるボタンのイベント（ヘッダー）
    document.getElementById('close-modal-header')?.addEventListener('click', () => {
        overlay.remove();
    });
    ...
    // 実際の誤字報告データを読み込む（仮実装）
    const workId = location.href.match(/\/works\/(\d+)/)?.[1];
    const episodeId = location.href.match(/\/episodes\/(\d+)/)?.[1];
    if (!workId || !episodeId) {
        console.error('Work ID or Episode ID not found in URL.');
        return;
    }

    await loadEpisodeErrorReports(workId, episodeId);
}

function getEpisodePageTitle(): string {
    ...
    const inputElement = document.querySelector('#episodeTitle-input') as HTMLInputElement;
...
function createEpisodeErrorReportCard(errorReport: ErrorReport, paragraphs: ChangedParagraph[]): HTMLElement {
    ...
    if (!isResolved) {
        ...
        markResolvedButton.addEventListener('click', async () =>{
            ...
            if (confirmResolve) {
                await handleEpisodeResolveError(workId, episodeId, errorReport.error_id);
            }
        });
    }
...
function createEpisodeEmptyState(): HTMLElement {
...
function createEpisodeErrorState(): HTMLElement {
    const container = document.createElement('div');
...
async function loadEpisodeErrorReports(workId: string, episodeId: string) {
    ...
    try {
        const response = await fetch(`${EpisodeApiBaseUrl}/works/${workId}/episodes/${episodeId}/errors`);
        if (!response.ok) {
        ...
        const data: ErrorReport[] = (
            ((await response.json()) ?? []) as ErrorReport[]).sort((a, b) => b.id - a.id);
        ...
        // エラー報告を表示
        data.forEach((errorReport: ErrorReport) => {
            const changedParagraphs: ChangedParagraph[] = JSON.parse(errorReport.error);
